...
ModelEarthquake modelEarthquakeFromJson(String str) =>
    ModelEarthquake.fromJson(json.decode(str));

String modelEarthquakeToJson(ModelEarthquake data) =>
    json.encode(data.toJson());

class ModelEarthquake {
  String type;
  Metadata metadata;
  ...
  factory ModelEarthquake.fromJson(Map<String, dynamic> json) =>
      ModelEarthquake(
        type: json["type"],
  ...
  static Future<List<Feature>> getFeature(int minMagnitude) async {
    var apiResult = await http.get(
        "https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&limit=10&minmagnitude=" +
            minMagnitude.toString());
    if (apiResult.statusCode == 200) {
      var data = json.decode(apiResult.body);
      List<Feature> feature = ModelEarthquake.fromJson(data).features;
      return feature;
    } else {
      throw Exception();
    }
...
enum MagType { ML, MD, MB, MWR, MWW, MB_LG }
...
enum Net { CI, NC, HV, US, NN, AK, UW, PR, MB, OK }
